...
export class MediaSystem {
  ...
  selectCoveringOutlets(storyType) {
    ...
    return allOutlets.filter(outlet => {
      ...
      const hasSpecialty = outlet.specialties.some(specialty => 
        this.storyMatchesSpecialty(storyType, specialty)
      );
  ...
  applyStoryEffects(story, gameState) {
    ...
    if (effects.mediaAttention) {
      this.newsCycle.mediaAttention = Math.max(0, Math.min(100,
  ...
  calculateScandalProbability(gameState) {
    ...
    const recentScandals = this.scandals.filter(s => 
      s.status === 'active' || (s.resolvedWeek && 
  ...
  triggerScandal(scandal, gameState) {
    ...
    this.newsCycle.mediaAttention = Math.min(100, 
  ...
  getScandalResponseEffects(response, scandal) {
    ...
    const severityMultiplier = scandal.severity === 'high' ? 1.5 : 
                              scandal.severity === 'medium' ? 1.0 : 0.7;
   ...
   * Update public opinion based on media coverage
   */
  updatePublicOpinion(gameState) {
    // Media influence on public opinion
    const mediaInfluence = this.calculateMediaInfluence();
